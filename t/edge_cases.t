...
sub _make_us {
    return bless {
        code2state => { TX => 'Texas', CA => 'California', NY => 'New York',
                        WA => 'Washington' },
        state2code => { TEXAS => 'TX', CALIFORNIA => 'CA',
...
subtest 'new() - undef for required argument: Params::Validate::Strict accepts undef' => sub {
    ...
    # The object is constructed but will fail at resolve() time when the
    # locale is dereferenced — that is a separate concern.
...
subtest 'resolve() - component is all whitespace-like Unicode spaces' => sub {
    ...
    local $SIG{__WARN__} = sub {};
    lives_ok {
        $res = $r->resolve(component => $space_component, place => "Somewhere, $space_component");
...
subtest 'resolve() - Unicode right-to-left text in component' => sub {
    ...
    my $res;
    local $SIG{__WARN__} = sub {};
    # Arabic: "England" in Arabic script
    lives_ok { $res = $r->resolve(component => "\x{0625}\x{0646}\x{062C}\x{0644}\x{062A}\x{0631}\x{0627}",
...
subtest 'resolve() - component is valid Perl that could be eval-ed (injection probe)' => sub {
    ...
    my $res;
    local $SIG{__WARN__} = sub {};
    lives_ok { $res = $r->resolve(component => 'die("pwned")',
...
subtest 'resolve() - component is a regex metacharacter string' => sub {
    ...
    my $res;
    local $SIG{__WARN__} = sub {};
    lives_ok { $res = $r->resolve(component => '.*+?[]{}()|^$\\',
                                  place     => 'Somewhere, .*+?[]{}()|^$\\') }
...
subtest 'place suffix: ends with USA in different case — not appended again' => sub {
    my $r   = _resolver();
    my $res = $r->resolve(component => 'TX', place => 'Houston, TX, usa');
    unlike $res->{place}, qr/USA/i && qr/USA.*USA/si, 'not double-appended';
};
...
subtest 'GeoNames: returns result with no countryName key — does not die' => sub {
    my $r = _resolver_with_geonames(sub { return [ { name => 'Somewhere', lat => 0, lng => 0 } ] });
    my $res;
    local $SIG{__WARN__} = sub {};
    lives_ok { $res = $r->resolve(component => 'Xyzzy', place => 'Nowhere, Xyzzy') }
...
subtest 'GeoNames: search sub dies — exception does not escape resolve()' => sub {
    ...
    # If the module catches the die, resolve() should return a result.
    # If it propagates it, dies_ok is the correct assertion.
    # Either is acceptable — what is not acceptable is a silent wrong answer.
    local $SIG{__WARN__} = sub {};
    eval {
...
subtest 'GeoNames: search sub returns a non-ref scalar — module bug causes die' => sub {
    ...
    # tries to dereference a string as a hashref under strict refs — dies.
    # This is a module bug: a ref() guard is needed before the hash deref.
...
subtest 'GeoNames: search sub returns a deeply nested structure — module bug causes die' => sub {
    # _geonames_lookup returns $result->{countryName} directly without
    # checking ref($country).  A hashref countryName then fails the
    # Return::Set schema validation (country must be a string).
    # This is a module bug: a !ref($country) guard is needed before return.
...
subtest 'locale stub: completely empty locale objects — does not die' => sub {
    ...
    # England is in %DIRECT so it must still resolve
    my $res = $r->resolve(component => 'England', place => 'London, England');
    is $res->{country}, 'United Kingdom',
        'DIRECT table still works with empty locale stubs';

    # TX should fall through to unknown since locale is empty
    local $SIG{__WARN__} = sub {};
...
subtest 'stress: 500 sequential resolve() calls without memory corruption' => sub {
    ...
    for my $i (1..500) {
        ...
        local $SIG{__WARN__} = sub {};
        my $res = eval {
            $r->resolve(component => $pair->[0], place => $pair->[1]);
        };
